...
#             self.get_parameter("parking_attempt_limit")
#             .get_parameter_value()
#             .integer_value
#         )
...
#             else:
#                 self.get_logger().warn(
#                     "Meter search failed and no goals remain. Mission complete."
#                 )
#                 self.set_state(State.DONE)

#     def trajectory_reached_callback(self, msg: Bool):
#         if msg.data and self.state == State.NAVIGATING:
#             if self.parking_attempt_count >= self.parking_attempt_limit:
#                 self.get_logger().info(
#                     "Return/start goal reached after two parking attempts. Mission complete."
#                 )
#                 self.set_state(State.DONE)
#                 return
...
#                 if self.parking_attempt_count >= self.parking_attempt_limit:
#                     self.get_logger().info(
#                         f"Within 1.0m of return/start goal. Distance: {dist:.2f}. Mission complete."
#                     )
#                     self.set_state(State.DONE)
#                     return
...
#                 else:
#                     self.get_logger().info(
#                         "Parking hold complete and no goals remain. Mission complete."
#                     )
#                     self.set_state(State.DONE)
...
#             State.METER_SEARCH: [State.PARKING, State.NAVIGATING, State.OBSTACLE_PAUSE, State.DONE],
...
class State(Enum):
    ...
    PARKED = 6
    DONE = 7
    THREE_POINT_TURN = 8 # New state for turning around
...
class BoatingExecutive(Node):
    def __init__(self):
        ...
        # Keeping parking subscriptions active but they will essentially be ignored
        # in the main state machine loop for now.
        self.create_subscription(
            Bool,
            "/parking_success",
            self.parking_success_callback,
        ...
        self.get_logger().info("State Decider Initialized (Parking Bypassed Mode)")
    ...
    def handle_goal_reached(self):
        if len(self.goals) > 0:
            self.current_goal = self.goals.pop(0)
            self.get_logger().info(f"Moving to next goal. {len(self.goals)} goals left.")
            self.set_state(State.NAVIGATING)
        else:
            if not self.is_returning:
    ...
    def loop(self):
        ...
        if self.state == State.WAITING:
            ...
            # Simple time-based open-loop 3-point turn
            elapsed = (self.get_clock().now() - self.turn_start_time).nanoseconds * 1e-9

            if elapsed < 2.5:
                # Phase 1: Hard Left, Forward
                self.publish_drive_command(1.0, 0.34)
            elif elapsed < 5.5:
                # Phase 2: Hard Right, Reverse
                self.publish_drive_command(-1.0, -0.34)
            elif elapsed < 7.5:
                # Phase 3: Center steering, pull forward to align
                self.publish_drive_command(1.0, 0.0)
            else:
                self.hit_the_brakes()
                self.is_returning = True

                # Reverse the goals, skipping the very first one (which is our current location)
                if len(self.all_goals) > 1:
                    self.goals = list(reversed(self.all_goals))[1:]
                else:
                    self.goals = list(reversed(self.all_goals))

                self.get_logger().info("3-Point turn complete. Beginning return journey.")
    ...
    def set_state(self, new_state):
        ...
        allowed = {
            State.WAITING: [State.NAVIGATING],
            State.NAVIGATING: [State.THREE_POINT_TURN, State.OBSTACLE_PAUSE, State.DONE],
            State.THREE_POINT_TURN: [State.NAVIGATING, State.OBSTACLE_PAUSE, State.DONE],
            State.OBSTACLE_PAUSE: [State.NAVIGATING, State.THREE_POINT_TURN],
            State.METER_SEARCH: [],
            State.PARKING: [],
            State.PARKED: [],
            State.DONE: []
