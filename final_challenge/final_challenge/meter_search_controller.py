...
class MeterSearchController(Node):
    ...
    3. Once 0.5 m away, back up until 1.0 m away.
    4. If still no parking-meter detection/state transition happened, perform
    ...
    def __init__(self):
        ...
        self.declare_parameter("back_up_speed", -0.20)
        self.declare_parameter("k_turn_speed", 0.25)
        ...
        self.near_wall_distance = float(self.get_parameter("near_wall_distance").value)
        self.far_wall_distance = float(self.get_parameter("far_wall_distance").value)
        ...
        self.wall_angle_gain = float(self.get_parameter("wall_angle_gain").value)
        ...
        self.forward_search_angle = float(self.get_parameter("forward_search_angle").value)
        self.k_turn_forward_duration = float(self.get_parameter("k_turn_forward_duration").value)
        self.k_turn_reverse_duration = float(self.get_parameter("k_turn_reverse_duration").value)
        self.max_k_turn_cycles = int(self.get_parameter("max_k_turn_cycles").value)
        ...
        self.get_logger().info("Meter Search Controller Initialized")
    ...
    def state_callback(self, msg):
        ...
        if previous_state != "METER_SEARCH" and self.current_state == "METER_SEARCH":
            self.reset_search()
            self.get_logger().info("Meter search active.")
    ...
    def scan_callback(self, scan):
        ...
        self.closest_wall_distance = float(valid_ranges[closest_idx])
        self.closest_wall_angle = float(valid_angles[closest_idx])
    ...
    def approach_wall(self):
        if self.closest_wall_distance is None:
            self.publish_drive_command(0.0, 0.0)
            self.get_logger().warn("Meter search waiting for valid lidar wall distance.")
            return
        ...
        self.publish_drive_command(self.approach_speed, steering)

    def back_up_from_wall(self):
        if self.closest_wall_distance is not None and self.closest_wall_distance >= self.far_wall_distance:
        ...
        self.publish_drive_command(self.back_up_speed, steering)
    ...
    def k_turn_forward(self):
        ...
        self.publish_drive_command(self.k_turn_speed, self.max_steering_angle)
    ...
    def set_phase(self, phase):
        self.phase = phase
        self.phase_start_time = self.now_sec()
        self.get_logger().info(f"Meter search phase -> {phase.name}")
    ...
    def publish_failure_once(self):
        ...
        self.get_logger().warn("Meter search failed after K-turn attempts.")
